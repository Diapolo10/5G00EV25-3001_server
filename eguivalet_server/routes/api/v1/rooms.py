...
async def get_public_rooms(db: Session = Depends(get_db)) -> list[Room]:
...
async def post_new_room(room: Room, db: Session = Depends(get_db)) -> Room:
    """Creates a new room"""
...
async def get_room_by_id(room_id: UUID, db: Session = Depends(get_db)) -> Room:
...
async def post_message_by_id(room_id: UUID, message: Message, db: Session = Depends(get_db)) -> Message:
...
async def delete_room_by_id(room_id: UUID, db: Session = Depends(get_db)) -> None:
...
async def get_messages(room_id: UUID, db: Session = Depends(get_db)) -> list[Message]:
...
async def get_message_by_id(room_id: UUID, message_id: UUID, db: Session = Depends(get_db)) -> Message:
    """Fetches the specified message"""
...
async def delete_message_by_id(room_id: UUID, message_id: UUID, db: Session = Depends(get_db)) -> None:
